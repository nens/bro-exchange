...
def gen_groundwatermonitoringnet(data, net, nsmap,count):
    ...
    broId = etree.SubElement(GroundwaterMonitoringNet, ("{%s}" % nsmap['gldcom']) + 'broId', nsmap=nsmap)    
...
def gen_monitoringpoint(data, point, nsmap, count):   
    ...
    broId = etree.SubElement(GroundwaterMonitoringTube, ("{%s}" % nsmap['gldcom']) + 'broId', nsmap=nsmap)    
    broId.text = data['monitoringPoints'][point]['broId']

    tubeNumber = etree.SubElement(GroundwaterMonitoringTube, ("{%s}" % nsmap['gldcom']) + 'tubeNumber', nsmap=nsmap)    
...
def gen_metadata_parameters(data, nsmap, codespacemap):
    ...
    arglist = {'principalInvestigator':'optional',
               'observationType':'obligated'}

    check_missing_args(data['metadata']['parameters'], arglist, 'gen_metadata_parameters')    
    ...
    parameterlist['principalInvestigator']=etree.Element(("{%s}" % nsmap['wml2']) + 'parameter', nsmap=nsmap) 
    ...
    if 'principalInvestigator' in parameters:
        if type(data['metadata']['parameters']['principalInvestigator'])!=dict:
            chamberOfCommerceNumber = etree.SubElement(principalInvestigator_omvalue,("{%s}" % nsmap['gldcom']) + 'chamberOfCommerceNumber', nsmap=nsmap)
            chamberOfCommerceNumber.text = str(data['metadata']['parameters']['principalInvestigator'])
        else:
            if 'chamberOfCommerceNumber' in list(data['metadata']['parameters']['principalInvestigator'].keys()):
                 chamberOfCommerceNumber = etree.SubElement(principalInvestigator_omvalue,("{%s}" % nsmap['gldcom']) + 'chamberOfCommerceNumber', nsmap=nsmap)
                 chamberOfCommerceNumber.text = str(data['metadata']['parameters']['principalInvestigator']['chamberOfCommerceNumber'])
            elif 'europeanCompanyRegistrationNumber' in list(data['metadata']['parameters']['principalInvestigator'].keys()):
                europeanCompanyRegistrationNumber = etree.SubElement(principalInvestigator_omvalue,("{%s}" % nsmap['gldcom']) + 'europeanCompanyRegistrationNumber', nsmap=nsmap)
                europeanCompanyRegistrationNumber.text = str(data['metadata']['parameters']['principalInvestigator']['europeanCompanyRegistrationNumber'])                 
                 
    # observation_type_metadata
    parameterlist['observationType']=etree.Element(("{%s}" % nsmap['wml2']) + 'parameter', nsmap=nsmap) 
...
def gen_metadata(data, nsmap, codespacemap):
    ...
    ObservationMetadata = etree.SubElement(metadata, ("{%s}" % nsmap['wml2']) + 'ObservationMetadata', nsmap=nsmap)  
    
    # Contact
    contact = etree.SubElement(ObservationMetadata, ("{%s}" % nsmap['gmd']) + 'contact', nsmap=nsmap) 
    CI_ResponsibleParty = etree.SubElement(contact, ("{%s}" % nsmap['gmd']) + 'CI_ResponsibleParty', nsmap=nsmap) 
    organisationName = etree.SubElement(CI_ResponsibleParty, ("{%s}" % nsmap['gmd']) + 'organisationName', nsmap=nsmap)  
    CharacterString = etree.SubElement(organisationName, ("{%s}" % nsmap['gco']) + 'CharacterString', nsmap=nsmap)  
    role = etree.SubElement(CI_ResponsibleParty, ("{%s}" % nsmap['gmd']) + 'role', nsmap=nsmap)
    
    if 'contact' in data['metadata'].keys():
        CI_RoleCode = etree.SubElement(role, ("{%s}" % nsmap['gmd']) + 'CI_RoleCode', nsmap=nsmap,
    ...
    if 'identificationInfo' in data['metadata'].keys():
        identificationInfo = etree.SubElement(ObservationMetadata, ("{%s}" % nsmap['gmd']) + 'identificationInfo', nsmap=nsmap,
                                              attrib ={
                                                  ("{%s}" % nsmap['gco'])+'nilReason':data['metadata']['identificationInfo']
                                                  }) 
    else:
        identificationInfo = etree.SubElement(ObservationMetadata, ("{%s}" % nsmap['gmd']) + 'identificationInfo', nsmap=nsmap,
                                              attrib ={
                                                  ("{%s}" % nsmap['gco'])+'nilReason':'unknown'
                                                  })   
    # status
    if 'status' in data['metadata'].keys(): 
        ...
        if data['metadata']['parameters']['observationType'] != 'controlemeting':
            raise Exception("argument 'status' obligated in observation metadata if not'controlemeting' as observationType")             
...
def gen_phenomenontime(data, nsmap, codespacemap, count):
    ...
    try:
        ...
        endPosition = str(pd.DataFrame(data['result'])['time'][len(pd.DataFrame(data['result']))-1])[:10]    
    except:
        raise Exception('Error: phenomenonTime cannot be derived from timeseries')
    
    phenomenonTime = etree.Element(("{%s}" % nsmap['om']) + 'phenomenonTime', nsmap=nsmap)    
    ...
    beginPosition_ = etree.SubElement(TimePeriod, ("{%s}" % nsmap['gml']) + 'beginPosition', nsmap=nsmap) 
    beginPosition_.text = beginPosition
    
    endPosition_ = etree.SubElement(TimePeriod, ("{%s}" % nsmap['gml']) + 'endPosition', nsmap=nsmap) 
...
def gen_resulttime(data, nsmap, codespacemap, count):
    ...
    timePosition = etree.SubElement(TimeInstant, ("{%s}" % nsmap['gml']) + 'timePosition', nsmap=nsmap) 
...
def gen_procedure_parameters(data, nsmap, codespacemap):
    ...
    if 'airPressureCompensationType' in parameters:
        parameterlist['airPressureCompensationType']=etree.Element(("{%s}" % nsmap['wml2']) + 'parameter', nsmap=nsmap) 
        airPressureCompensationType_namevalue = etree.SubElement(parameterlist['airPressureCompensationType'],("{%s}" % nsmap['om']) + 'NamedValue', nsmap=nsmap) 
    ...
    if 'evaluationProcedure' in parameters:
        parameterlist['evaluationProcedure']=etree.Element(("{%s}" % nsmap['wml2']) + 'parameter', nsmap=nsmap) 
        evaluationProcedure_namevalue = etree.SubElement(parameterlist['evaluationProcedure'],("{%s}" % nsmap['om']) + 'NamedValue', nsmap=nsmap) 
    ...
    if 'measurementInstrumentType' in parameters:
        parameterlist['measurementInstrumentType']=etree.Element(("{%s}" % nsmap['wml2']) + 'parameter', nsmap=nsmap) 
        measurementInstrumentType_namevalue = etree.SubElement(parameterlist['measurementInstrumentType'],("{%s}" % nsmap['om']) + 'NamedValue', nsmap=nsmap) 
...
def gen_procedure(data, nsmap, codespacemap):
    ...
    processType = etree.SubElement(ObservationProcess, ("{%s}" % nsmap['wml2']) + 'processType', nsmap=nsmap,
                                           attrib={
                                               ("{%s}" % nsmap['xlink'])+'href':processTypestr
                                               }
                                           )
    ...
    processReference = etree.SubElement(ObservationProcess, ("{%s}" % nsmap['wml2']) + 'processReference', nsmap=nsmap,
                                           attrib={
                                               ("{%s}" % nsmap['xlink'])+'href':processReferencestr
                                               }
                                           )                
...
def gen_point_metadata_qualifiers(data, rec, nsmap, codespacemap, count):
    ...
    if 'StatusQualityControl' in metadata:
        qualifierlist['StatusQualityControl']=etree.Element(("{%s}" % nsmap['wml2']) + 'qualifier', nsmap=nsmap) 
...
def gen_point_metadata(data, rec, nsmap, codespacemap, count):
    ...
    TVPMeasurementMetadata  = etree.SubElement(metadata, ("{%s}" % nsmap['wml2']) + 'TVPMeasurementMetadata', nsmap=nsmap) 
    ...
    if 'interpolationType' not in rec['metadata'].keys():
        ...
        #try:        
            qualifiers = gen_point_metadata_qualifiers(data, rec, nsmap, codespacemap, count)
            ...
            if 'interpolationType' in rec['metadata'].keys():
                interpolationType  = etree.SubElement(TVPMeasurementMetadata, ("{%s}" % nsmap['wml2']) + 'interpolationType', nsmap=nsmap,
                                                                 attrib = {
                                                                             ("{%s}" % nsmap['xlink'])+'href':"http://www.opengis.net/def/waterml/2.0/interpolationType/{}".format(rec['metadata']['interpolationType'])                                                               
                                                                     }) 
            if 'censoredReason' in rec['metadata'].keys():
                censoredReason  = etree.SubElement(TVPMeasurementMetadata, ("{%s}" % nsmap['wml2']) + 'censoredReason', nsmap=nsmap,
                                                                 attrib = {
                                                                             ("{%s}" % nsmap['xlink'])+'href':"http://www.opengis.net/def/nil/OGC/0/{}".format(rec['metadata']['censoredReason'])                                                               
                                                                     })                
        #except:
...
def gen_point(data, rec, nsmap, codespacemap, count):
    ...
    MeasurementTVP  = etree.SubElement(point, ("{%s}" % nsmap['wml2']) + 'MeasurementTVP', nsmap=nsmap) 

    time  = etree.SubElement(MeasurementTVP, ("{%s}" % nsmap['wml2']) + 'time', nsmap=nsmap)
    time.text = rec['time']
    
    if rec['value'] != 'None':
        value  = etree.SubElement(MeasurementTVP, ("{%s}" % nsmap['wml2']) + 'value', nsmap=nsmap,
													attrib = {'uom':'m'})
