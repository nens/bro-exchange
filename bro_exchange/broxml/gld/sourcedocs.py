...
def gen_gld_startregistration(data, nsmap, codespacemap):
    ...
    arglist = {
        "objectIdAccountableParty": "optional",
    ...
    GLD_StartRegistration = etree.SubElement(
        sourceDocument,
        "GLD_StartRegistration",
        attrib={("{%s}" % nsmap["gml"]) + "id": "id_0001"},
    )
    ...
    for arg in data.keys():
        if arg not in constructables:
            if arg in codespacemap.keys():
                GLD_StartRegistration_subelements[arg] = etree.SubElement(
                    GLD_StartRegistration, arg, codeSpace=codespacemap[arg]
                )
                GLD_StartRegistration_subelements[arg].text = str(data[arg])
            else:
                GLD_StartRegistration_subelements[arg] = etree.SubElement(
                    GLD_StartRegistration, arg
                )
                GLD_StartRegistration_subelements[arg].text = str(data[arg])
            ...
            if arg == "groundwaterMonitoringNets":
                if len(data[arg]) < 1:
                    ...
                    for net in range(len(data[arg])):
                        (
                            GLD_StartRegistration_subelements[
                                f"groundwaterMonitoringNet{str(net)}"
                            ],
                            count,
                        ) = gen_groundwatermonitoringnet(data, net, nsmap, count)
                        GLD_StartRegistration.append(
                            GLD_StartRegistration_subelements[
                                f"groundwaterMonitoringNet{str(net)}"
                            ]
                        )
                ...
                if len(data[arg]) != 1:
                    raise Exception(
                        "One monitoringpoint should be provided, no more or no less"
                    )
                else:
                    for point in range(len(data[arg])):
                        (
                            GLD_StartRegistration_subelements[
                                f"monitoringPoint{str(point)}"
                            ],
                            count,
                        ) = gen_monitoringpoint(data, point, nsmap, count)
                        GLD_StartRegistration.append(
                            GLD_StartRegistration_subelements[
                                f"monitoringPoint{str(point)}"
                            ]
...
def gen_gld_addition(data, nsmap, codespacemap):
    ...
    for arg in arglist.keys():
        if arg in data.keys():
            if arg == "metadata":
                OM_Observation_subelements["metadata"] = gen_metadata(
                    data, nsmap, codespacemap
                )
                OM_Observation.append(OM_Observation_subelements["metadata"])
            elif arg == "phenomenonTime":
                (
                    OM_Observation_subelements["phenomenonTime"],
                    count,
                ) = gen_phenomenontime(data, nsmap, codespacemap, count)
                OM_Observation.append(OM_Observation_subelements["phenomenonTime"])
            elif arg == "resultTime":
                OM_Observation_subelements["resultTime"], count = gen_resulttime(
                    data, nsmap, codespacemap, count
                )
                ...
                if type(data["procedure"]) == str:
                    OM_Observation_subelements["procedure"] = etree.SubElement(
                        OM_Observation,
                        ("{%s}" % nsmap["om"]) + "procedure",
                        nsmap=nsmap,
                        attrib={("{%s}" % nsmap["xlink"]) + "href": data["procedure"]},
                    )

                # 2: creation of new procedure with given input

                elif type(data["procedure"]) == dict:
                    OM_Observation_subelements["procedure"] = gen_procedure(
                        data, nsmap, codespacemap
                    )
                    OM_Observation.append(OM_Observation_subelements["procedure"])
                else:
                    raise Exception(
                        "Error: invalid input type for procedure, should be 'dict' or 'str' "
                    )

            elif arg == "observedProperty":
                observedProperty = etree.SubElement(
                    OM_Observation,
                    ("{%s}" % nsmap["om"]) + "observedProperty",
                    nsmap=nsmap,
                )
                ...
                if type(data["result"]) != list:
                    raise Exception(
                        "Error: invalid input type for result, should be list with dictionaries"
                    )
                elif list(pd.DataFrame(data["result"]).columns) != [
                    "time",
                    "value",
                    "metadata",
                ]:
                    raise Exception(
                        "Error: invalid input fields for result, fields should be ['time','value','qualifiers']"
                    )
                else:
                    OM_Observation_subelements["result"], count = gen_result(
                        data, nsmap, codespacemap, count
                    )
