...
def gen_gmn_closure(data): 
    arglist =  {'endDateMonitoring':'obligated',}   
    ...
    check_missing_args(data, arglist, 'gen_gmn_construction')
    sourceDocument = etree.Element("sourceDocument") 
    GMN_MeasuringPointEndDate = etree.SubElement(sourceDocument,"GMN_Closure", 
                                        attrib = {            
                                        ("{%s}" % ns_regreq_map_gmn2['gml'])+'id': 'id_0001'})
    ...
    for arg in data.keys():
        if arg == 'endDateMonitoring':
            GMN_MeasuringPointEndDate_subelements[arg] = gen_enddate(data, ns_regreq_map_gmn2)
            GMN_MeasuringPointEndDate.append(GMN_MeasuringPointEndDate_subelements[arg])
