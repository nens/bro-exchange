...
def gen_gmw_construction(data, nsmap, codespacemap, sourcedoctype):
    ...
    if sourcedoctype == "GMW_Construction":
        sourceDocument = etree.Element(
            ("{%s}" % nsmap["ns"]) + "sourceDocument", nsmap=nsmap
        )
        GMW_Construction = etree.SubElement(
            sourceDocument, ("{%s}" % nsmap["ns"]) + sourcedoctype, nsmap=nsmap
        )

    else:
        GMW_Construction = etree.Element(
            ("{%s}" % nsmap["ns"]) + sourcedoctype, nsmap=nsmap
        )
            ...
            if arg == ordered_arg:
                if arg not in constructables:
                    if arg in codespacemap.keys():
                        GMW_Construction_subelements[arg] = etree.SubElement(
                            GMW_Construction,
                            ("{%s}" % nsmap["ns"]) + arg,
                            nsmap=nsmap,
                            codeSpace=codespacemap[arg],
                        )
                        GMW_Construction_subelements[arg].text = str(data[arg])
                    else:
                        GMW_Construction_subelements[arg] = etree.SubElement(
                            GMW_Construction, ("{%s}" % nsmap["ns"]) + arg, nsmap=nsmap
                        )
                        GMW_Construction_subelements[arg].text = str(data[arg])

                else:
                    if arg == "wellConstructionDate":
                        GMW_Construction_subelements[arg] = gen_wellconstructiondate(
                            data, nsmap
                        )
                        GMW_Construction.append(GMW_Construction_subelements[arg])

                    elif arg == "deliveredLocation":
                        GMW_Construction_subelements[arg] = gen_deliveredlocation(
                            data, nsmap, codespacemap
                        )
                        GMW_Construction.append(GMW_Construction_subelements[arg])
                        ...
                        GMW_Construction_subelements[
                            arg
                        ] = gen_deliveredverticalposition(data, nsmap, codespacemap)
                        GMW_Construction.append(GMW_Construction_subelements[arg])

                    elif arg == "monitoringTubes":
                        if len(data["monitoringTubes"]) < 1:
                            raise Exception(
                                "No monitoring tubes provided in input, at least 1 monitoringtube should be provided"
                            )
                        else:
                            for tube in range(len(data[arg])):
                                GMW_Construction_subelements[
                                    f"monitoringTube{str(tube)}"
                                ] = gen_monitoringtube(
                                    data, tube, nsmap, codespacemap, sourcedoctype
                                )
                                GMW_Construction.append(
                                    GMW_Construction_subelements[
                                        f"monitoringTube{str(tube)}"
                                    ]
                                )
...
def gen_gmw_wellheadprotector(data, nsmap, codespacemap, sourcedoctype):
    ...
    if sourcedoctype == "GMW_WellHeadProtector":
        ...
        GMW_WellHeadProtector = etree.SubElement(
            sourceDocument, ("{%s}" % nsmap["ns"]) + sourcedoctype, nsmap=nsmap
        )
    else:
        GMW_WellHeadProtector = etree.Element(
            ("{%s}" % nsmap["ns"]) + sourcedoctype, nsmap=nsmap
        )
    ...
    for ordered_arg in arglist.keys():
        for arg in data.keys():
            if arg == ordered_arg:
                if arg not in constructables:
                    if arg in codespacemap.keys():
                        GMW_WellHeadProtector_subelements[arg] = etree.SubElement(
                    ...
                    if arg == "eventDate":
                        GMW_WellHeadProtector_subelements[arg] = gen_eventdate(
                            data, nsmap
                        )
                        GMW_WellHeadProtector.append(
                            GMW_WellHeadProtector_subelements[arg]
                        )
...
def gen_gmw_lengthening_shortening(data, nsmap, codespacemap, sourcedoctype):
    ...
    if sourcedoctype == "GMW_Lengthening":
        ...
        GMW_Lengthening_Shortening = etree.SubElement(
            sourceDocument, ("{%s}" % nsmap["ns"]) + "GMW_Lengthening", nsmap=nsmap
        )
        ...
        GMW_Lengthening_Shortening = etree.SubElement(
            sourceDocument, ("{%s}" % nsmap["ns"]) + "GMW_Shortening", nsmap=nsmap
        )
        ammount = data["numberOfTubesShortened"]

    elif sourcedoctype == "shortening":
        GMW_Lengthening_Shortening = etree.Element(
            ("{%s}" % nsmap["ns"]) + "shortening", nsmap=nsmap
        )
        ammount = data["numberOfTubesShortened"]

    elif sourcedoctype == "shortening":
        GMW_Lengthening_Shortening = etree.Element(
            ("{%s}" % nsmap["ns"]) + "lengthening", nsmap=nsmap
        )
        ammount = data["numberOfTubesLengthened"]
    ...
    for ordered_arg in arglist.keys():
        for arg in data.keys():
            if arg == ordered_arg:
                if arg not in constructables:
                    if arg in codespacemap.keys():
                        GMW_Lengthening_Shortening_subelements[arg] = etree.SubElement(
                            GMW_Lengthening_Shortening,
                            ("{%s}" % nsmap["ns"]) + arg,
                            nsmap=nsmap,
                            codeSpace=codespacemap[arg],
                        )
                        GMW_Lengthening_Shortening_subelements[arg].text = str(
                            data[arg]
                        )
                    else:
                        GMW_Lengthening_Shortening_subelements[arg] = etree.SubElement(
                    ...
                    if arg == "eventDate":
                        GMW_Lengthening_Shortening_subelements[arg] = gen_eventdate(
                            data, nsmap
                        )
                        GMW_Lengthening_Shortening.append(
                            GMW_Lengthening_Shortening_subelements[arg]
                        )

                    elif arg == "monitoringTubes":
                        if len(data["monitoringTubes"]) < ammount:
                            raise Exception(
                                "Ammount of monitoringtubes should be equal to numberOfTubesLengthened"
                            )
                        else:
                            for tube in range(len(data[arg])):
                                GMW_Lengthening_Shortening_subelements[
...
def gen_gmw_groundlevel(data, nsmap, codespacemap, sourcedoctype):
    ...
    if sourcedoctype == "GMW_GroundLevel":
        ...
        GMW_GroundLevel = etree.SubElement(
            sourceDocument, ("{%s}" % nsmap["ns"]) + sourcedoctype, nsmap=nsmap
        )
    else:
        GMW_GroundLevel = etree.Element(
            ("{%s}" % nsmap["ns"]) + sourcedoctype, nsmap=nsmap
        )
    ...
    for ordered_arg in arglist.keys():
        for arg in data.keys():
            if arg == ordered_arg:
                if arg not in constructables:
                    if arg in codespacemap.keys():
                        GMW_GroundLevel_subelements[arg] = etree.SubElement(
                            GMW_GroundLevel,
                            ("{%s}" % nsmap["ns"]) + arg,
                            nsmap=nsmap,
                            codeSpace=codespacemap[arg],
                        ...
                        GMW_GroundLevel_subelements[arg] = etree.SubElement(
                            GMW_GroundLevel, ("{%s}" % nsmap["ns"]) + arg, nsmap=nsmap
                        )
                        GMW_GroundLevel_subelements[arg].text = str(data[arg])
                    ...
                    if arg == "eventDate":
                        ...
                        if "groundLevelPosition" in list(
                            data["deliveredVerticalPosition"].keys()
                        ):
                            groundLevelPosition = etree.SubElement(
                                GMW_GroundLevel,
                                ("{%s}" % nsmap["ns"]) + "groundLevelPosition",
                                nsmap=nsmap,
                                uom="m",
                            )
                        ...
                        if "groundLevelPositioningMethod" in list(
                            data["deliveredVerticalPosition"].keys()
                        ):
                            groundLevelPositioningMethod = etree.SubElement(
                                GMW_GroundLevel,
                                ("{%s}" % nsmap["ns"]) + "groundLevelPositioningMethod",
                                nsmap=nsmap,
                                codeSpace=codespacemap["groundLevelPositioningMethod"],
                            )
                            groundLevelPositioningMethod.text = data[
                                "deliveredVerticalPosition"
    ...
    for ordered_arg in arglist.keys():
        for arg in data.keys():
            if arg == ordered_arg:
                if arg not in constructables:
                    if arg in codespacemap.keys():
                        GMW_Owner_subelements[arg] = etree.SubElement(
                            GMW_Owner,
                            ("{%s}" % nsmap["ns"]) + arg,
                            nsmap=nsmap,
                            codeSpace=codespacemap[arg],
                        ...
                        GMW_Owner_subelements[arg] = etree.SubElement(
                            GMW_Owner, ("{%s}" % nsmap["ns"]) + arg, nsmap=nsmap
                        )
                        GMW_Owner_subelements[arg].text = str(data[arg])
...
def gen_gmw_positions(data, nsmap, codespacemap, sourcedoctype):
    ...
    if sourcedoctype == "GMW_Positions":
        ...
        GMW_Positions = etree.SubElement(
            sourceDocument, ("{%s}" % nsmap["ns"]) + sourcedoctype, nsmap=nsmap
        )
    else:
        GMW_Positions = etree.Element(
            ("{%s}" % nsmap["ns"]) + sourcedoctype, nsmap=nsmap
        )
    ...
    for ordered_arg in arglist.keys():
        for arg in data.keys():
            if arg == ordered_arg:
                if arg not in constructables:
                    if arg in codespacemap.keys():
                        GMW_Positions_subelements[arg] = etree.SubElement(
                            GMW_Positions,
                            ("{%s}" % nsmap["ns"]) + arg,
                            nsmap=nsmap,
                            codeSpace=codespacemap[arg],
                        ...
                        GMW_Positions_subelements[arg] = etree.SubElement(
                            GMW_Positions, ("{%s}" % nsmap["ns"]) + arg, nsmap=nsmap
                        )
                        GMW_Positions_subelements[arg].text = str(data[arg])
...
def gen_gmw_electrodestatus(data, nsmap, codespacemap, sourcedoctype):
    ...
    if sourcedoctype == "GMW_ElectrodeStatus":
        ...
        GMW_ElectrodeStatus = etree.SubElement(
            sourceDocument, ("{%s}" % nsmap["ns"]) + sourcedoctype, nsmap=nsmap
        )

    else:
        GMW_ElectrodeStatus = etree.Element(
            ("{%s}" % nsmap["ns"]) + sourcedoctype, nsmap=nsmap
        )
    ...
    for ordered_arg in arglist.keys():
        for arg in data.keys():
            if arg == ordered_arg:
                if arg not in constructables:
                    if arg in codespacemap.keys():
                        GMW_ElectrodeStatus_subelements[arg] = etree.SubElement(
                            GMW_ElectrodeStatus,
                            ("{%s}" % nsmap["ns"]) + arg,
                            nsmap=nsmap,
                            codeSpace=codespacemap[arg],
                    ...
                    if arg == "eventDate":
                        GMW_ElectrodeStatus_subelements[arg] = gen_eventdate(
                            data, nsmap
                        )
                        ...
                        if len(data["electrodes"]) < ammount:
                            raise Exception(
                                "Ammount of electrodes should be equal to numberOfElectrodesChanged"
                            )
                        else:
                            for electrode in range(len(data[arg])):
                                GMW_ElectrodeStatus_subelements[
                                    f"electrode{str(electrode)}"
                                ] = adjust_electrode(
                                    data, electrode, nsmap, codespacemap
                                )
                                GMW_ElectrodeStatus.append(
                                    GMW_ElectrodeStatus_subelements[
                                        f"electrode{str(electrode)}"
                                    ]
                                )
...
def gen_gmw_maintainer(data, nsmap, codespacemap, sourcedoctype):
    ...
    if sourcedoctype == "GMW_Maintainer":
        ...
        GMW_Maintainer = etree.SubElement(
            sourceDocument, ("{%s}" % nsmap["ns"]) + sourcedoctype, nsmap=nsmap
        )

    else:
        GMW_Maintainer = etree.Element(
            ("{%s}" % nsmap["ns"]) + sourcedoctype, nsmap=nsmap
        )
    ...
    for ordered_arg in arglist.keys():
        for arg in data.keys():
            if arg == ordered_arg:
                if arg not in constructables:
                    if arg in codespacemap.keys():
                        GMW_Maintainer_subelements[arg] = etree.SubElement(
                            GMW_Maintainer,
                            ("{%s}" % nsmap["ns"]) + arg,
                            nsmap=nsmap,
                            codeSpace=codespacemap[arg],
                        ...
                        GMW_Maintainer_subelements[arg] = etree.SubElement(
                            GMW_Maintainer, ("{%s}" % nsmap["ns"]) + arg, nsmap=nsmap
                        )
                        GMW_Maintainer_subelements[arg].text = str(data[arg])
                else:
                    if arg == "eventDate":
...
def gen_gmw_tubestatus(data, nsmap, codespacemap, sourcedoctype):
    ...
    if sourcedoctype == "GMW_TubeStatus":
        ...
        GMW_TubeStatus = etree.SubElement(
            sourceDocument, ("{%s}" % nsmap["ns"]) + sourcedoctype, nsmap=nsmap
        )
    else:
        GMW_TubeStatus = etree.Element(
            ("{%s}" % nsmap["ns"]) + sourcedoctype, nsmap=nsmap
        )
    ...
    for ordered_arg in arglist.keys():
        for arg in data.keys():
            if arg == ordered_arg:
                if arg not in constructables:
                    if arg in codespacemap.keys():
                        GMW_TubeStatus_subelements[arg] = etree.SubElement(
                            GMW_TubeStatus,
                            ("{%s}" % nsmap["ns"]) + arg,
                            nsmap=nsmap,
                            codeSpace=codespacemap[arg],
                        ...
                        GMW_TubeStatus_subelements[arg] = etree.SubElement(
                            GMW_TubeStatus, ("{%s}" % nsmap["ns"]) + arg, nsmap=nsmap
                        )
                        GMW_TubeStatus_subelements[arg].text = str(data[arg])
                else:
                    if arg == "eventDate":
...
def gen_gmw_insertion(data, nsmap, codespacemap, sourcedoctype):
    ...
    if sourcedoctype == "GMW_Insertion":
        ...
        GMW_Insertion = etree.SubElement(
            sourceDocument, ("{%s}" % nsmap["ns"]) + sourcedoctype, nsmap=nsmap
        )

    else:
        GMW_Insertion = etree.Element(
            ("{%s}" % nsmap["ns"]) + sourcedoctype, nsmap=nsmap
        )
    ...
    for ordered_arg in arglist.keys():
        for arg in data.keys():
            if arg == ordered_arg:
                if arg not in constructables:
                    if arg in codespacemap.keys():
                        GMW_Insertion_subelements[arg] = etree.SubElement(
                            GMW_Insertion,
                            ("{%s}" % nsmap["ns"]) + arg,
                            nsmap=nsmap,
                            codeSpace=codespacemap[arg],
                        ...
                        GMW_Insertion_subelements[arg] = etree.SubElement(
                            GMW_Insertion, ("{%s}" % nsmap["ns"]) + arg, nsmap=nsmap
                        )
                        GMW_Insertion_subelements[arg].text = str(data[arg])
                else:
                    if arg == "eventDate":
                    ...
                    if arg == "insertedPartLength":
                        insertedPartLength = etree.SubElement(
                            GMW_Insertion,
                            ("{%s}" % nsmap["ns"]) + "insertedPartLength",
                            nsmap=nsmap,
                            uom="m",
                        )
                    ...
                    if arg == "insertedPartDiameter":
                        insertedPartDiameter = etree.SubElement(
                            GMW_Insertion,
                            ("{%s}" % nsmap["ns"]) + "insertedPartDiameter",
                            nsmap=nsmap,
                            uom="mm",
                        )
                    ...
                    if arg == "insertedPartMaterial":
                        insertedPartMaterial = etree.SubElement(
                            GMW_Insertion,
                            ("{%s}" % nsmap["ns"]) + "insertedPartMaterial",
                            nsmap=nsmap,
...
def gen_gmw_shift(data, nsmap, codespacemap, sourcedoctype):
    arglist = {"eventDate": "obligated", "deliveredVerticalPosition": "obligated"}
    ...
    for ordered_arg in arglist.keys():
        for arg in data.keys():
            if arg == ordered_arg:
                if arg not in constructables:
                    if arg in codespacemap.keys():
                        GMW_Shift_subelements[arg] = etree.SubElement(
                            GMW_Shift,
                            ("{%s}" % nsmap["ns"]) + arg,
                            nsmap=nsmap,
                            codeSpace=codespacemap[arg],
                        ...
                        GMW_Shift_subelements[arg] = etree.SubElement(
                            GMW_Shift, ("{%s}" % nsmap["ns"]) + arg, nsmap=nsmap
                        )
                        GMW_Shift_subelements[arg].text = str(data[arg])
                else:
                    if arg == "eventDate":
    ...
    for ordered_arg in arglist.keys():
        for arg in data.keys():
            if arg == ordered_arg:
                if arg not in constructables:
                    if arg in codespacemap.keys():
                        GMW_Removal_subelements[arg] = etree.SubElement(
                            GMW_Removal,
                            ("{%s}" % nsmap["ns"]) + arg,
                            nsmap=nsmap,
                            codeSpace=codespacemap[arg],
                        ...
                        GMW_Removal_subelements[arg] = etree.SubElement(
                            GMW_Removal, ("{%s}" % nsmap["ns"]) + arg, nsmap=nsmap
                        )
                        GMW_Removal_subelements[arg].text = str(data[arg])
                else:
...
def gen_gmw_groundlevelmeasuring(data, nsmap, codespacemap, sourcedoctype):
    ...
    if sourcedoctype == "GMW_GroundLevelMeasuring":
        ...
        GMW_GroundLevelMeasuring = etree.SubElement(
            sourceDocument, ("{%s}" % nsmap["ns"]) + sourcedoctype, nsmap=nsmap
        )
    else:
        GMW_GroundLevelMeasuring = etree.Element(
            ("{%s}" % nsmap["ns"]) + sourcedoctype, nsmap=nsmap
        )
    ...
    for ordered_arg in arglist.keys():
        for arg in data.keys():
            if arg == ordered_arg:
                if arg not in constructables:
                    if arg in codespacemap.keys():
                        GMW_GroundLevelMeasuring_subelements[arg] = etree.SubElement(
                    ...
                    if arg == "eventDate":
                        GMW_GroundLevelMeasuring_subelements[arg] = gen_eventdate(
                            data, nsmap
                        )
                        GMW_GroundLevelMeasuring.append(
                            GMW_GroundLevelMeasuring_subelements[arg]
                        )
...
def gen_gmw_positionsmeasuring(data, nsmap, codespacemap, sourcedoctype):
    ...
    if sourcedoctype == "GMW_PositionsMeasuring":
        ...
        GMW_PositionsMeasuring = etree.SubElement(
            sourceDocument, ("{%s}" % nsmap["ns"]) + sourcedoctype, nsmap=nsmap
        )
    else:
        GMW_PositionsMeasuring = etree.Element(
            ("{%s}" % nsmap["ns"]) + sourcedoctype, nsmap=nsmap
        )
    ...
    for ordered_arg in arglist.keys():
        for arg in data.keys():
            if arg == ordered_arg:
                if arg not in constructables:
                    if arg in codespacemap.keys():
                        GMW_PositionsMeasuring_subelements[arg] = etree.SubElement(
                    ...
                    if arg == "eventDate":
                        GMW_PositionsMeasuring_subelements[arg] = gen_eventdate(
                            data, nsmap
                        )
                        GMW_PositionsMeasuring.append(
                            GMW_PositionsMeasuring_subelements[arg]
                        )
...
def gen_gmw_constructionwithhistory(data, nsmap, codespacemap, sourcedoctype):
    ...
    sourceDocument_main = etree.Element(
        ("{%s}" % nsmap["ns"]) + "sourceDocument", nsmap=nsmap
    )
    GMW_ConstructionWithHistory = etree.SubElement(
        sourceDocument_main,
        ("{%s}" % nsmap["ns"]) + "GMW_ConstructionWithHistory",
        nsmap=nsmap,
    )

    # Step 1: create construction
    construction = gen_gmw_construction(
        data["construction"], nsmap, codespacemap, "construction"
    )
    ...
    for event in range(len(data["events"])):
        intermediate_events[str(event)] = etree.SubElement(
            GMW_ConstructionWithHistory,
            ("{%s}" % nsmap["ns"]) + "intermediateEvent",
            nsmap=nsmap,
        )

        if data["events"][event]["srcdoc"] == "GMW_Owner":
            sourceDocument = gen_gmw_owner(
                data["events"][event]["eventdata"], nsmap, codespacemap, "owner"
            )
        ...
        if data["events"][event]["srcdoc"] == "GMW_GroundLevel":
            sourceDocument = gen_gmw_groundlevel(
                data["events"][event]["eventdata"], nsmap, codespacemap, "groundLevel"
            )
            intermediate_events[str(event)].append(sourceDocument)
            GMW_ConstructionWithHistory.append(intermediate_events[str(event)])
        ...
        if data["events"][event]["srcdoc"] == "GMW_Positions":
            sourceDocument = gen_gmw_positions(
                data["events"][event]["eventdata"], nsmap, codespacemap, "positions"
            )
            intermediate_events[str(event)].append(sourceDocument)
            GMW_ConstructionWithHistory.append(intermediate_events[str(event)])
        ...
        if data["events"][event]["srcdoc"] == "GMW_Maintainer":
            sourceDocument = gen_gmw_maintainer(
                data["events"][event]["eventdata"], nsmap, codespacemap, "maintainer"
            )
            intermediate_events[str(event)].append(sourceDocument)
            GMW_ConstructionWithHistory.append(intermediate_events[str(event)])
        ...
        if data["events"][event]["srcdoc"] == "GMW_TubeStatus":
            sourceDocument = gen_gmw_tubestatus(
                data["events"][event]["eventdata"], nsmap, codespacemap, "tubeStatus"
            )
            intermediate_events[str(event)].append(sourceDocument)
            GMW_ConstructionWithHistory.append(intermediate_events[str(event)])
        ...
        if data["events"][event]["srcdoc"] == "GMW_Insertion":
            sourceDocument = gen_gmw_insertion(
                data["events"][event]["eventdata"], nsmap, codespacemap, "insertion"
            )
            intermediate_events[str(event)].append(sourceDocument)
            GMW_ConstructionWithHistory.append(intermediate_events[str(event)])
        ...
        if data["events"][event]["srcdoc"] == "GMW_Shift":
            sourceDocument = gen_gmw_shift(
                data["events"][event]["eventdata"], nsmap, codespacemap, "shift"
            )
            intermediate_events[str(event)].append(sourceDocument)
            GMW_ConstructionWithHistory.append(intermediate_events[str(event)])
