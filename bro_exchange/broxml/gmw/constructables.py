...
def gen_removaldate(data, nsmap):
    ...
    wellRemovalDate = etree.Element(("{%s}" % nsmap['ns']) + 'wellRemovalDate', nsmap=nsmap)
    date = etree.SubElement(wellRemovalDate, ("{%s}" % nsmap['ns1']) + 'date', nsmap=nsmap)
...
def gen_materialused(data, tube, nsmap, codespacemap, sourcedoctype):
    ...
    if sourcedoctype in ['GMW_Construction','construction']: 
        ...
        arglist = {'tubePackingMaterial':'optional',
    ...
    if 'tubePackingMaterial' in list(data['monitoringTubes'][tube]['materialUsed'].keys()):
...
def adjust_electrode(data, electrode, nsmap, codespacemap):
    ...
    check_missing_args(targetdata, arglist, 'gen_monitoringtube, tube with index {}, geoOhmCable with index {}, electrode with index {}'.format(str(targetdata['tubeNumber']),
                                                                                                                                                str(targetdata['cableNumber']),
                                                                                                                                                str(targetdata['electrodeNumber'])))
    
    electrode = etree.Element(("{%s}" % nsmap['ns']) + 'electrode', nsmap=nsmap)

    tubeNumber = etree.SubElement(electrode, ("{%s}" % nsmap['ns']) + 'tubeNumber', nsmap = nsmap)
    tubeNumber.text = str(targetdata['tubeNumber'])

    cableNumber = etree.SubElement(electrode, ("{%s}" % nsmap['ns']) + 'cableNumber', nsmap = nsmap)
    cableNumber.text = str(targetdata['cableNumber'])

    electrodeNumber = etree.SubElement(electrode, ("{%s}" % nsmap['ns']) + 'electrodeNumber', nsmap = nsmap)
    electrodeNumber.text = str(targetdata['electrodeNumber'])
    
    electrodeStatus = etree.SubElement(electrode, ("{%s}" % nsmap['ns']) + 'electrodeStatus', nsmap = nsmap, codeSpace=codespacemap['electrodeStatus'])
    electrodeStatus.text = str(targetdata['electrodeStatus'])
...
def gen_monitoringtube(data, tube, nsmap,codespacemap, sourcedoctype):
    ...
    elif sourcedoctype in ['GMW_Positions','GMW_PositionsMeasuring',
                           'positions','positionsMeasuring']:
        arglist = {'tubeNumber':'obligated',
                    'tubeTopPosition':'obligated',
                    'tubeTopPositioningMethod':'obligated'} 
    ...
    if sourcedoctype in ['GMW_Construction','construction']:
        ...
        if 'materialUsed' in list(data['monitoringTubes'][tube].keys()):            
            materialUsed = gen_materialused(data, tube, nsmap,codespacemap, sourcedoctype)
        ...
        if sourcedoctype not in ['GMW_Shortening','shortening']:
            ...
            if 'tubeMaterial' in list(data['monitoringTubes'][tube]['materialUsed'].keys()):        
                tubeMaterial = etree.SubElement(monitoringTube, ("{%s}" % nsmap['ns']) + 'tubeMaterial', nsmap = nsmap, codeSpace=codespacemap['tubeMaterial'])
                tubeMaterial.text = str(data['monitoringTubes'][tube]['materialUsed']['tubeMaterial'])
    
            if 'glue' in list(data['monitoringTubes'][tube]['materialUsed'].keys()):        
                glue = etree.SubElement(monitoringTube, ("{%s}" % nsmap['ns']) + 'glue', nsmap = nsmap, codeSpace="urn:bro:gmw:Glue")
                glue.text = str(data['monitoringTubes'][tube]['materialUsed']['glue'])    

        if 'plainTubePartLength' in list(data['monitoringTubes'][tube]['plainTubePart'].keys()):        
            plainTubePartLength = etree.SubElement(monitoringTube, ("{%s}" % nsmap['ns']) + 'plainTubePartLength', nsmap = nsmap, uom="m")
            plainTubePartLength.text = str(data['monitoringTubes'][tube]['plainTubePart']['plainTubePartLength'])
